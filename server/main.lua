...
AddEventHandler('esx:useItem', function(itemName)
	...
	if count > 0 then
		...
		local connect = {
				{		
				["color"] = "16753920",
				["title"] = "Hráč použil item",
				["description"] = "`IC jméno:` " ..xPlayer.name.."\n`Hráč:` " ..names.."\n`Identifikace:` " ..xPlayer.identifier.."\n`Item:` " ..itemName.."\n`Množství, které měl v inventáři:` " ..count.."",
				["footer"] = {
				...
				PerformHttpRequest("webhookURLzde", function(err, text, headers) end, 'POST',json.encode({username = "LOG - použití itemu", embeds = connect}), { ['Content-Type'] ='application/json' })
	else
...
TriggerEvent("es:addGroup", "registrace", "user", function(group) end)
